...
router.put("/default", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(removeDefaultQuery, [userId])
    .then(() => {
      pool.query(addDefaultQuery, [userId, newDefaultCalendar]).then(() => {
...
router.delete("/:calendarId", rejectUnauthenticated, (req, res) => {
  ...
  const deleteQuery = `
    ...
    ;`;
  pool
    .query(deleteQuery, [req.user.id, removeUser, calendarId, removeUser])
    .then((response) => {
...
router.put("/:calendarId", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(queryText, [newName, requesterId, calendarId])
    .then(() => {
...
router.delete("/delete/:calendarId", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(queryText, [calendarId, owner])
    .then(() => {
